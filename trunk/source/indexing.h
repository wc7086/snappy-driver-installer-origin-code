...
// Exporter
const int num_windowsbits=2;
const int num_windowver=7;
const int num_hwidpos=32;

class datum
{
private:
    char data[num_hwidpos*num_windowver*num_windowsbits];
...
class Exporter
{
    std::set<std::string> hwids;
    std::set<std::string>::iterator hwids_it;
...
public:
    ...
    bool Advance();
    void SetWindowVer();
    const char*GetHWID(){return hwids_it->c_str();}
};
extern Exporter ex;
